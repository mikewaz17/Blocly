...
//    }
    ...
    //#58 Added three different entries into RssFeedTable for testing.
    ...
    //#59 New Fragment which will display one RSS item at a time.
    ...
    /*#58 Added two more methods for feed recovery from the database. The first method recovers a single feed
     * with it's row ID. The second method calls on fetchAllFeeds method to recover all RSS feed in the database
     * to the caller.
     */
    ...
    /*#58 Added fetchNewItemsForFeed, it performs a GetFeedsNetworkRequest and comes back with a new list of RSS items.
     *Also checks to see if items are in the database before inserting them.
     */
    ...
    /*#58 checking for errors for GetFeedsNetworkRequest. The method inserts the RSS item into the database
     * it's row identifier with all of the items listed.
     */
}
